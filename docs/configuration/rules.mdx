...
> **Note:** A `bundled_packages` value set *inside* a rule's `config` is only honored for individual (non-grouped) updates — it is ignored on the [grouping path](#grouping-into-a-single-pull-request). Since a matching rule already groups the packages it matches into one pull request, to bundle additional packages, add them to the rule's `matchRules` instead, or set `bundled_packages` at the root level.
